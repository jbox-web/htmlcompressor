...
module HtmlCompressor
  class Compressor # rubocop:disable Metrics/ClassLength
    ...
    DEFAULT_REGEX_FLAGS = Regexp::MULTILINE | Regexp::IGNORECASE
    EMPTY_PATTERN = Regexp.new('\\s')
    SKIP_PATTERN = Regexp.new('<!--\\s*\\{\\{\\{\\s*-->(.*?)<!--\\s*\\}\\}\\}\\s*-->', DEFAULT_REGEX_FLAGS)
    COND_COMMENT_PATTERN = Regexp.new('(<!(?:--)?\\[[^\\]]+?\\]>)(.*?)(<!\\[[^\\]]+\\]-->)', DEFAULT_REGEX_FLAGS)
    COMMENT_PATTERN = Regexp.new('<!---->|<!--[^\\[].*?-->', DEFAULT_REGEX_FLAGS)
    INTERTAG_PATTERN_TAG_TAG = Regexp.new('>\\s+<', DEFAULT_REGEX_FLAGS)
    INTERTAG_PATTERN_TAG_CUSTOM = Regexp.new('>\\s+%%%~', DEFAULT_REGEX_FLAGS)
    INTERTAG_PATTERN_CUSTOM_TAG = Regexp.new('~%%%\\s+<', DEFAULT_REGEX_FLAGS)
    INTERTAG_PATTERN_CUSTOM_CUSTOM = Regexp.new('~%%%\\s+%%%~', DEFAULT_REGEX_FLAGS)
    MULTISPACE_PATTERN = Regexp.new('\\s+', DEFAULT_REGEX_FLAGS)
    TAG_END_SPACE_PATTERN = Regexp.new('(<(?:[^>]+?))(?:\\s+?)(/?>)', DEFAULT_REGEX_FLAGS)
    ...
    PRE_PATTERN = Regexp.new('(<pre[^>]*?>)(.*?)(</pre>)', DEFAULT_REGEX_FLAGS)
    TA_PATTERN = Regexp.new('(<textarea[^>]*?>)(.*?)(</textarea>)', DEFAULT_REGEX_FLAGS)
    SCRIPT_PATTERN = Regexp.new('(<script[^>]*?>)(.*?)(</script>)', DEFAULT_REGEX_FLAGS)
    STYLE_PATTERN = Regexp.new('(<style[^>]*?>)(.*?)(</style>)', DEFAULT_REGEX_FLAGS)
    TAG_PROPERTY_PATTERN = Regexp.new('(\\s\\w+)\\s*=\\s*(?=[^<]*?>)', Regexp::IGNORECASE)
    CDATA_PATTERN = Regexp.new('\\s*<!\\[CDATA\\[(.*?)\\]\\]>\\s*', DEFAULT_REGEX_FLAGS)
    DOCTYPE_PATTERN = Regexp.new('<!DOCTYPE[^>]*>', DEFAULT_REGEX_FLAGS)
    TYPE_ATTR_PATTERN = Regexp.new("type\\s*=\\s*([\\\"']*)(.+?)\\1", DEFAULT_REGEX_FLAGS)
    JS_TYPE_ATTR_PATTERN = Regexp.new("(<script[^>]*)type\\s*=\\s*([\"']*)(?:text|application)\/javascript\\2([^>]*>)", DEFAULT_REGEX_FLAGS)
    JS_LANG_ATTR_PATTERN = Regexp.new("(<script[^>]*)language\\s*=\\s*([\"']*)javascript\\2([^>]*>)", DEFAULT_REGEX_FLAGS)
    STYLE_TYPE_ATTR_PATTERN = Regexp.new("(<style[^>]*)type\\s*=\\s*([\"']*)text/(?:style|css)\\2([^>]*>)", DEFAULT_REGEX_FLAGS)
    LINK_TYPE_ATTR_PATTERN = Regexp.new("(<link[^>]*)type\\s*=\\s*([\"']*)text/(?:css|plain)\\2([^>]*>)", DEFAULT_REGEX_FLAGS)
    LINK_REL_ATTR_PATTERN = Regexp.new("<link(?:[^>]*)rel\\s*=\\s*([\"']*)(?:alternate\\s+)?stylesheet\\1(?:[^>]*)>", DEFAULT_REGEX_FLAGS)
    FORM_METHOD_ATTR_PATTERN = Regexp.new("(<form[^>]*)method\\s*=\\s*([\"']*)get\\2([^>]*>)", DEFAULT_REGEX_FLAGS)
    INPUT_TYPE_ATTR_PATTERN = Regexp.new("(<input[^>]*)type\\s*=\\s*([\"']*)text\\2([^>]*>)", DEFAULT_REGEX_FLAGS)
    BOOLEAN_ATTR_PATTERN = Regexp.new("(<\\w+[^>]*[\"' ])(checked|selected|disabled|readonly)\\s*=\\s*([\"']*)\\w*\\3([^>]*>)", DEFAULT_REGEX_FLAGS)
    EVENT_JS_PROTOCOL_PATTERN = Regexp.new('^javascript:\\s*(.+)', DEFAULT_REGEX_FLAGS)
    HTTP_PROTOCOL_PATTERN = Regexp.new("(<[^>]+?(?:href|src|cite|action)\\s*=\\s*['\"])http:(//[^>]+?>)", DEFAULT_REGEX_FLAGS)
    HTTPS_PROTOCOL_PATTERN = Regexp.new("(<[^>]+?(?:href|src|cite|action)\\s*=\\s*['\"])https:(//[^>]+?>)", DEFAULT_REGEX_FLAGS)
    REL_EXTERNAL_PATTERN = Regexp.new("<(?:[^>]*)rel\\s*=\\s*([\"']*)(?:alternate\\s+)?external\\1(?:[^>]*)>", DEFAULT_REGEX_FLAGS)
    ...
    SURROUNDING_SPACES_MIN_PATTERN = Regexp.new('\\s*(</?(?:' + BLOCK_TAGS_MIN.gsub(',', '|') + ')(?:>|[\\s/][^>]*>))\\s*', DEFAULT_REGEX_FLAGS)
    SURROUNDING_SPACES_MAX_PATTERN = Regexp.new('\\s*(</?(?:' + BLOCK_TAGS_MAX.gsub(',', '|') + ')(?:>|[\\s/][^>]*>))\\s*', DEFAULT_REGEX_FLAGS)
    SURROUNDING_SPACES_ALL_PATTERN = Regexp.new('\\s*(<[^>]+>)\\s*', DEFAULT_REGEX_FLAGS)
    ...
    def preserve_blocks(html, preBlocks, taBlocks, scriptBlocks, styleBlocks, eventBlocks, condCommentBlocks, skipBlocks, lineBreakBlocks, userBlocks) # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/PerceivedComplexity,Metrics/ParameterLists
      ...
      html = html.gsub(SKIP_PATTERN) do |match|
        if ::Regexp.last_match(1).strip.length > 0
          skipBlocks << match
          skipBlockIndex += 1
      ...
      html = html.gsub(COND_COMMENT_PATTERN) do |_match|
        if ::Regexp.last_match(2).strip.length > 0
          index += 1
          condCommentBlocks << (::Regexp.last_match(1) + condCommentCompressor.compress(::Regexp.last_match(2)) + ::Regexp.last_match(3))
      ...
      html = html.gsub(EVENT_PATTERN1) do |_match|
        if ::Regexp.last_match(2).strip.length > 0
          eventBlocks << ::Regexp.last_match(2)
          index += 1
          ::Regexp.last_match(1) + message_format(TEMP_EVENT_BLOCK, index) + ::Regexp.last_match(3)
        else
          ''
        end
      end

      html = html.gsub(EVENT_PATTERN2) do |_match|
        if ::Regexp.last_match(2).strip.length > 0
          eventBlocks << ::Regexp.last_match(2)
          index += 1
          ::Regexp.last_match(1) + message_format(TEMP_EVENT_BLOCK, index) + ::Regexp.last_match(3)
      ...
      # preserve PRE tags
      index = -1
      html = html.gsub PRE_PATTERN do |_match|
        if ::Regexp.last_match(2).strip.length > 0
          index += 1
          preBlocks << ::Regexp.last_match(2)
          ::Regexp.last_match(1) + message_format(TEMP_PRE_BLOCK, index) + ::Regexp.last_match(3)
        else
          ''
        end
      end
      ...
      html = html.gsub(SCRIPT_PATTERN) do |match|
        group_1 = ::Regexp.last_match(1)
        group_2 = ::Regexp.last_match(2)
        group_3 = ::Regexp.last_match(3)
      ...
      html = html.gsub(STYLE_PATTERN) do |match|
        if ::Regexp.last_match(2).strip.length > 0
          styleBlocks << ::Regexp.last_match(2)
          index += 1
          ::Regexp.last_match(1) + message_format(TEMP_STYLE_BLOCK, index) + ::Regexp.last_match(3)
        else
          match
        end
      end
      ...
      if @options[:preserve_line_breaks]
        index = -1
        html = html.gsub(LINE_BREAK_PATTERN) do |_match|
          lineBreakBlocks << ::Regexp.last_match(1)
          index += 1
    ...
    def return_blocks(html, preBlocks, taBlocks, scriptBlocks, styleBlocks, eventBlocks, condCommentBlocks, skipBlocks, lineBreakBlocks, userBlocks) # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/PerceivedComplexity,Metrics/ParameterLists
      ...
      if @options[:preserve_line_breaks]
        html = html.gsub(TEMP_LINE_BREAK_PATTERN) do |_match|
          i = ::Regexp.last_match(1).to_i
      ...
      html = html.gsub(TEMP_TEXT_AREA_PATTERN) do |_match|
        i = ::Regexp.last_match(1).to_i
        if taBlocks.size > i
          taBlocks[i]
        else
      ...
      html = html.gsub(TEMP_STYLE_PATTERN) do |_match|
        i = ::Regexp.last_match(1).to_i
      ...
      html = html.gsub(TEMP_SCRIPT_PATTERN) do |_match|
        i = ::Regexp.last_match(1).to_i
      ...
      # put PRE blocks back
      html = html.gsub TEMP_PRE_PATTERN do |_match|
        i = ::Regexp.last_match(1).to_i
        if preBlocks.size > i
      ...
      html = html.gsub(TEMP_EVENT_PATTERN) do |_match|
        i = ::Regexp.last_match(1).to_i
      ...
      html = html.gsub(TEMP_COND_COMMENT_PATTERN) do |_match|
        i = ::Regexp.last_match(1).to_i
    ...
    def process_script_blocks(scriptBlocks)
      return unless @options[:compress_javascript]
    ...
    def process_style_blocks(styleBlocks)
      return unless @options[:compress_css]
    ...
    def process_event_blocks(eventBlocks)
      return unless @options[:remove_javascript_protocol]
    ...
    def compress_css_styles(source)
      ...
      css_compressor = get_css_compressor(css_compressor) if css_compressor.is_a?(Symbol)
    ...
    def remove_spaces_inside_tags(html)
      ...
      if @options[:remove_spaces_inside_tags]
        ...
        html.gsub!(TAG_END_SPACE_PATTERN) do |_match|
          group_1 = ::Regexp.last_match(1)
          group_2 = ::Regexp.last_match(2)
    ...
    def remove_quotes_inside_tags(html)
      if @options[:remove_quotes]
        html = html.gsub(TAG_QUOTE_PATTERN) do |_match|
    ...
    def remove_surrounding_spaces(html)
      ...
      unless @options[:remove_surrounding_spaces].nil?
        pattern =
          case @options[:remove_surrounding_spaces].downcase
          when BLOCK_TAGS_MIN
          ...
          when ALL_TAGS
            ...
            Regexp.new('\\s*(</?(?:' + @options[:remove_surrounding_spaces].gsub(',', '|') + ')(?:>|[\\s/][^>]*>))\\s*', DEFAULT_REGEX_FLAGS)
          end
    ...
    def message_format(message, *params)
      message.gsub(/\{(\d+),number,#\}/) do
        params[::Regexp.last_match(1).to_i]
      end
    end
  end
end
